...
int main ()
{
  ...
  double min_steer = -1.2;

  // initialize pid throttle
  double kp_throttle = 0.08;
  double ki_throttle = 0.01;
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          // Since what we are interested in is the direction of the car and not the steering angle, the error on the direction is computed 
          // and the PID gives a proportionnal response to this error 
          ...
          // error is the diff between the actual speed and the desired speed the difference is computed this way 
          // so the sign is coherent with the PID equation to give a adequate response to the error
          error_throttle = velocity - desired_velocity;
